...
ui::Button* TRBaseScene::addCommonBtn(const Vec2& pos, const std::string& text, COMMON_CALLBACK callback)
{
    ...
    int tag = _btnAI++;
    _cbMap[tag] = callback;
    return _defaultLayer->decorateButtonEx({"images/common_btn_bg.png", pos, .4f, text},
                                    [this,tag](){this->_cbMap[tag]();});
}

void TRBaseScene::addCommonLabel(const cocos2d::Vec2 &pos, const std::string &text, int size)
{
    auto lb = Label::createWithTTF(text, uic::font_zh, size);
